...
module.exports = async args => {
  ...
  fileNames.forEach(fileName => {
    ...
    if (retryExecutedLength >= args.retryCount) {
      const logWarning = ['move-files', fileName, 'All retryCounts used up', 'Retry count limit', args.retryCount, 'Current retries', retryLength]
      if (!args.silentErrors || !content.retries[args.retryCount - 1].lastNotification) {
        ...
        logWarning.push('Notification silenced')
      }
      ...
      try {
        ...
        // content.retries[content.retries - 1].notified = true
        updateFile(args.errorPath, fileName, content)
