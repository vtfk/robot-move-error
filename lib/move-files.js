...
const notify = async (args, body) => {
  if (args.sendNotification) {
    const notification = generateNotification(args, body)
      ...
      if (err) {
        ...
        files.forEach(fileName => {
          ...
          try {
            ...
            logger('info', ['move-files', 'File updated with new retryCount and moved', fileName, 'retryCount', content.retryCount])
            notify(args, {
              level: 'warn',
            ...
            logger('error', ['move-files', 'Failed to move or update file with new retryCount', fileName, content.retryCount, err])
          }
        })
